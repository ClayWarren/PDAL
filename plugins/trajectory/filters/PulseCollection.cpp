...
PulseCollection::PulseCollection(const Args& args)
    : m_args(args), m_first(true), m_lowhigh(0)
{
}

PulseCollection::~PulseCollection() {}

void PulseCollection::Add(double time, const Eigen::Vector3d& r, int nr, int rn,
                          double angle)
{
    ...
    if (m_first)
    {
        ...
        m_timeOrigin =
            std::floor(time / 10) * 10; // Round left to lowest 10 value.
        m_rOrigin = (Eigen::floor(r.array() / 100) * 100)
                        .matrix(); // Round to nearest 100
        m_timeMin = time;
        m_first = false;
    }
...
{
    auto shouldRegister = [](const std::vector<Pulse>& buf, double curTime,
                             double interval) -> bool
    {
        if (buf.empty())
            return false;
        return std::floor(buf.front().t / interval) !=
               std::floor(curTime / interval);
    };
...
void PulseCollection::registerSingle(const std::vector<Pulse>& buf)
{
    ...
    // low can end up -1 and high can end up == buf.size(), but the because
    // integer division rounds to 0, this will always yield a value in range.
    mid = (low + high) / 2;
    ...
    if (low >= 0 && high < (int)buf.size() &&
        ...
        pulses.back().n = (buf[mid].ang - buf[low].ang > 0 ? 1 : -1) *
                          (buf[high].r - buf[low].r);
}

bool PulseCollection::EstimatedPositionVelocity(double t, Eigen::Vector3d& r,
                                                Eigen::Vector3d& v) const
{
    std::vector<Pulse> psub;

    // Extract pulses in a window around t. Adjust pulse times to be relative to
    // t.
    ...
    for (int l = 0, h = 0; l < k; ++l)
    {
        ...
        A(2 * h + 1, 4) = nz * p.t;
        A(2 * h + 1, 5) = -ny * p.t;
        A(2 * h + 0, 1) = A(2 * h + 0, 4) = A(2 * h + 1, 0) = A(2 * h + 1, 3) =
            0;
...
void PulseCollection::InitializeTrajectory()
{
    ...
    int num =
        int(std::ceil((m_timeMax - m_timeOrigin) / m_args.tblock) - tstart) - 1;
    if (num < 1)
    ...
    if (!traj.fillmissing(true))
        throw pdal_error("PulseCollection: too few pulses for initial estimate "
                         "of trajectory");
    ...
    for (int i = 0; i <= num; ++i)
    {
        ...
        attitude.r[i] =
            Eigen::Vector2d(std::atan2(traj.r[ip](0) - traj.r[im](0),
                            ...
                            std::isnan(m_args.fixedpitch)
                                ? 0.0
                                : degreesToRadians(m_args.fixedpitch));
...
void PulseCollection::Solve()
{
    ...
    for (const Pulse& p : pulses)
    {
        ...
        if (p.MultiReturn())
        {
            ...
            ceres::LossFunction* loss_function = new ceres::ScaledLoss(
                new ceres::CauchyLoss(m_args.dr), m_args.multiweight,
                ceres::TAKE_OWNERSHIP);
            problem.AddResidualBlock(
                cost_function, loss_function, traj.r[i].data(),
                traj.v[i].data(), traj.r[i + 1].data(), traj.v[i + 1].data());
        ...
        else
        {
            ceres::CostFunction* cost_function =
                new ceres::AutoDiffCostFunction<ScanAngleError,
                                                2, // number of residuals
                                                3, 3, 3, 3, 2, 2, 2,
                                                2> // data for cubic fit
                (new ScanAngleError(p, t, m_args.pitchweight,
                                    degreesToRadians(m_args.fixedpitch)));
            ceres::LossFunction* loss_function = new ceres::ScaledLoss(
                new ceres::CauchyLoss(degreesToRadians(m_args.dang)),
                m_args.scanweight, ceres::TAKE_OWNERSHIP);
            problem.AddResidualBlock(
                cost_function, loss_function, traj.r[i].data(),
                traj.v[i].data(), traj.r[i + 1].data(), traj.v[i + 1].data(),
                attitude.r[i].data(), attitude.v[i].data(),
                attitude.r[i + 1].data(), attitude.v[i + 1].data());
        ...
        for (int i = 1; i < num; ++i)
        {
            ...
            ceres::LossFunction* loss_function =
                (ceres::LossFunction*)(new ceres::ScaledLoss(
                    nullptr, m_args.accelweight, ceres::TAKE_OWNERSHIP));
    ...
    if (m_args.attaccelweight > 0)
    {
        for (int i = 1; i < num; ++i)
        {
            ...
            ceres::LossFunction* loss_function =
                (ceres::LossFunction*)(new ceres::ScaledLoss(
                    nullptr, m_args.attaccelweight, ceres::TAKE_OWNERSHIP));
            problem.AddResidualBlock(
                cost_function, loss_function, attitude.r[i - 1].data(),
                attitude.v[i - 1].data(), attitude.v[i].data(),
                attitude.r[i + 1].data(), attitude.v[i + 1].data());
        }
    }
    ...
    if (m_args.clampweight > 0 || m_args.straddleweight > 0)
    {
        ...
        {
            double w =
                traj.missing[i] ? m_args.straddleweight : m_args.clampweight;
            if (w <= 0)
                continue;
            ceres::CostFunction* cost_function =
            ...
            ceres::LossFunction* loss_function =
                (ceres::LossFunction*)(new ceres::ScaledLoss(
                    nullptr, w, ceres::TAKE_OWNERSHIP));
            problem.AddResidualBlock(cost_function, loss_function,
                                     traj.r[i - 1].data(), traj.v[i - 1].data(),
    ...
    if (m_args.attclampweight > 0)
    {
        ...
        for (int i = 1; i < num; ++i)
        {
            ...
            ceres::LossFunction* loss_function =
                (ceres::LossFunction*)(new ceres::ScaledLoss(
                    nullptr, m_args.attclampweight, ceres::TAKE_OWNERSHIP));
            problem.AddResidualBlock(
                cost_function, loss_function, attitude.r[i - 1].data(),
                attitude.v[i - 1].data(), attitude.r[i].data(),
                attitude.r[i + 1].data(), attitude.v[i + 1].data());
        }
    }
...
Eigen::Vector3d PulseCollection::Trajectory(double t, Eigen::Vector3d& v,
...
{
    Eigen::Vector2d p =
        radiansToDegrees(attitude.position(t - m_timeOrigin, v));
