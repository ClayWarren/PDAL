...
use gdal_sys::{GDALDatasetH, GDALGetRasterBand, GDALRasterIO, GDALDataType, CPLErr, GDALAccess, GDALRWFlag};
...
impl Raster {
    ...
    pub fn read_band(&self, band_idx: i32, width: usize, height: usize, buffer: &mut [f64]) -> Result<(), String> {
        unsafe {
    ...
    pub fn read_at(&self, x: f64, y: f64, buffer: &mut [f64]) -> Result<(), String> {
        unsafe {
            ...
            if gdal_sys::GDALInvGeoTransform(transform.as_mut_ptr(), inv_transform.as_mut_ptr()) == 0 {
                return Err("Failed to invert geo transform".to_string());
            }
            
            let pixel = (inv_transform[0] + x * inv_transform[1] + y * inv_transform[2]).floor() as i32;
            let line = (inv_transform[3] + x * inv_transform[4] + y * inv_transform[5]).floor() as i32;
...
impl Vector {
    ...
    pub fn get_features(&self, layer_idx: i32) -> Result<Vec<(String, i32)>, String> {
        unsafe {
            ...
            loop {
                ...
                if !geom.is_null() {
                    ...
                    if gdal_sys::OGR_G_ExportToWkt(geom, &mut wkt_ptr) == CPLErr::CE_None {
                        let wkt = std::ffi::CStr::from_ptr(wkt_ptr).to_string_lossy().into_owned();
                        gdal_sys::VSIFree(wkt_ptr as *mut _);
                        
                        // Default to first field for now
                        let val = gdal_sys::OGR_F_GetFieldAsInteger(feature, 1);
                        result.push((wkt, val));
                    }
                }
