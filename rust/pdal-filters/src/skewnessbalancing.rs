...
mod tests {
    ...
    #[test]
    fn marks_all_ground_when_skewness_never_becomes_positive() {
        ...
        for z in -5..5 {
            ...
            view.set_f64(id, &DimId::Z, z as f64);
        }
        ...
        for id in 0..output.len() {
            assert_eq!(output.get_f64(id, &DimId::Classification), 2.0);
        }
    }

    #[test]
    fn sorts_by_height_before_classifying() {
        let mut layout = PointLayout::new();
        layout.register(DimId::Z, DimType::F64);
        layout.register(DimId::Classification, DimType::U8);
        let mut view = PointView::new(Rc::new(layout));
        ...
        assert_eq!(output.get_f64(0, &DimId::Z), -1.0);
        assert_eq!(output.get_f64(1, &DimId::Z), 1.0);
        assert_eq!(output.get_f64(2, &DimId::Z), 3.0);
    }
}
