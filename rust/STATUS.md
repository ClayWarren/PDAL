...
| Text I/O | done | Existing C++ reader/writer unit-test shapes pass through the Rust-backed path, and installed-PDAL regression coverage exists for scoped workflows. Reader input can open local files and GDAL VSI/HTTP(S) text sources. |
| PCD I/O | in progress | Existing C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII, binary, binary-compressed, precision, streaming, and double-field coverage. ASCII writer output is streamable in the Rust pipeline executor, using a temporary row stream so the final `POINTS` header remains correct; binary and binary-compressed writes remain materialized. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Broader installed-PDAL parity coverage can still grow. |
| PTS/PTX readers | in progress | Existing C++ reader unit-test shapes pass through the Rust-backed path, including Leica ASCII fixture behavior and installed-PDAL regressions. Reader input can open local files and GDAL VSI/HTTP(S) text sources. |
...
| PLY I/O | in progress | C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII/binary reads, ASCII/binary writes, mesh faces, precision/dim typing, and `#` flex filenames. ASCII single-file vertex writer output is streamable in the Rust pipeline executor, using a temporary row stream so the final vertex count remains correct; binary, mesh-face, and `#` fan-out writes remain materialized. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Broader installed-PDAL and uncommon PLY fixture coverage can still grow. |
...
| QFIT reader | in progress | Existing C++ reader unit-test shapes pass through the Rust-backed path for deterministic NASA ATM QFIT binary fixtures, with reader input able to open local files and GDAL VSI/HTTP(S) byte sources. |
| SBET/SMRMSG I/O | in progress | Existing C++ SBET reader/writer and SMRMSG reader unit-test shapes pass through the Rust-backed path for deterministic trajectory fixtures. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. |
...
| TerraSolid reader | in progress | Existing C++ reader unit-test shapes pass through the Rust-backed path for deterministic TerraSolid format 2 fixtures. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. `.bin` is not inferred because it conflicts with FBI. |
| Optech reader | in progress | Existing C++ reader unit-test shapes pass through the Rust-backed path, including deterministic Optech CSD fixture data and localized WGS84 georeference math. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. |
