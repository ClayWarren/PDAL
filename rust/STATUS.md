...
| PCD I/O | done | Existing C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII, binary, binary-compressed, precision, streaming, and double-field coverage. ASCII and uncompressed binary reader input plus ASCII and binary writer output are streamable in the Rust pipeline executor; the writer uses a temporary row stream so the final `POINTS` header remains correct. Binary-compressed reads/writes intentionally use the materialized executor because the format stores one whole compressed transposed payload. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Installed-PDAL parity coverage includes the decimation pipeline and compressed writer round-trip. |
