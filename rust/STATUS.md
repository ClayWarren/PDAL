...
| category | LOC | files |
|---|---:|---:|
| port-candidate | 23,014 | 210 |
| c-abi-backed | 38,182 | 337 |
| native-adapter | 1,428 | 10 |
| holdout | 339 | 4 |
| total | 62,963 | 561 |

Port-candidate backlog by area: `pdal` 9,332 · `io` 8,010 · `filters` 3,436 ·
`kernels` 2,236. `apps` and `tools` are now at 0 (apps is a thin entry-point
...
(The latest kernel sweep moved direct C++ `EvalKernel`/`GroundKernel`/
`MergeKernel`/`RandomKernel`/`SortKernel`/`SplitKernel`/`TileKernel` execution
through `pdal_rust_kernel_run`, reducing kernel port-candidate backlog by
933 LOC / 14 files while retaining the exported classes as compatibility
shells.)
