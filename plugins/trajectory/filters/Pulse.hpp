...
public:
    ...
    Pulse()
        : t(0), r(Eigen::Vector3d::Zero()), n(Eigen::Vector3d::Zero()), d(0),
          ang(0)
    {
    }

    Pulse(double _t, const Eigen::Vector3d& _r, double _ang = 0)
        : t(_t), r(_r), n(Eigen::Vector3d::Zero()), d(0),
          ang(degreesToRadians(_ang))
    {
    }
    Pulse(double _t, const Eigen::Vector3d& _f, const Eigen::Vector3d& _l,
          double _ang = 0)
...
public:
    FirstLastError(const Pulse& p, double t)
        : _t(t) // fractional time in block
          ,
          _r(p.r), _M(PerpProjector(p.n, p.d))
    {
    }
...
public:
    ScanAngleError(const Pulse& p, double t, double pitchweight = 1,
        ...
        : _t(t) // fractional time in block
          ,
          _ang(p.ang), _pitch(pitch), _pitchweight(pitchweight), _r(p.r),
                    ...
                    T* residuals) const
    {
        ...
        if (!_fixedpitch)
            a[1] =
                SplineFitScalar::EndPointCubic(am[1], bm[1], ap[1], bp[1], t);
        else
            a[1] = pitch;
