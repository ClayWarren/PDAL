...
| GitHub workflows | in progress | The Pixi workflow runs the Rust workspace guard through `pixi run -e dev rust-guard`; shared CI conda environments and Docker/Alpine build images now install Rust/Cargo explicitly for CMake builds that require the Rust C ABI. Pixi exposes named Rust/C++ parity audits (`rust-cpp-port-audit`, `rust-cpp-test-parity`) for CI wiring, but Linux, macOS, Windows, conda, and release workflows still need those gates enabled before the port is upstreamable. |
