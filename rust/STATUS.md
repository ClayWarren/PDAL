...
| category | LOC | files |
|---|---:|---:|
| port-candidate | 23,399 | 214 |
| c-abi-backed | 37,949 | 333 |
| native-adapter | 1,428 | 10 |
| holdout | 339 | 4 |
| total | 63,115 | 561 |

Port-candidate backlog by area: `pdal` 9,332 · `io` 8,010 · `filters` 3,436 ·
`kernels` 2,621. `apps` and `tools` are now at 0 (apps is a thin entry-point
...
(The latest kernel sweep moved direct C++ `EvalKernel`/`MergeKernel`/
`RandomKernel`/`SortKernel`/`SplitKernel` execution through
`pdal_rust_kernel_run`, reducing kernel port-candidate backlog by 548 LOC /
10 files while retaining the exported classes as compatibility shells.)
