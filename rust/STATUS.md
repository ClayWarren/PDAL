...
- EPT/COPC/STAC/remote breadth: transformed EPT preview point counts are
  Rust-pruned, but transformed preview bounds clipping, origin/polygon/OGR
  preview fallbacks, broad remote traversal, schema validation, object-store
  option parity, and accepted addon limitations remain.
...
| Unsafe Rust footprint | in progress | Current first-party Rust count, excluding `rust/target`, is 339 `unsafe { ... }` blocks, 566 `unsafe extern "C" fn` exports, 83 total `unsafe fn` helpers, two unsafe extern callback type aliases, no unsafe extern blocks, and one `unsafe impl`. Unsafe remains concentrated in `pdal-capi`, `pdal-native`, and Rust callers of the C ABI; keep new unsafe at C/native boundaries or tests that exercise those boundaries. |
...
- `pdal_io_ept_reader_test`: `inspect`, `inspectBounds`, `fullReadLaszip`,
  ...
  same-SRS bounds-backed preview upper-bound counts and clipped preview bounds,
  ...
  `writers.ept_addon` — keep working with the Rust read path. Transformed
  bounds, origin, polygon, and OGR spatial-filter preview remain C++ fallback.
