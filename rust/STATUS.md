...
| STAC reader | prototype | Local STAC Item/Collection/FeatureCollection traversal can read local and covered remote assets through already-ported readers. Preview supports item/date/bounds pruning, local structural validation for the schema-flag fixture shapes, and the GeoJSON OGR-boundary shape covered by existing C++ tests. Execution supports item/date/bounds/OGR/property filters and reader-specific args for local supported STAC documents. Mixed-reader merges in Rust normalize to a union layout before appending views. Rust collection follows Catalog links internally, but the C++ wrapper keeps Catalog roots, including the existing mixed EPT/COPC catalog workflow, on the legacy path until remote ranged COPC reads are ported. Full remote JSON-schema resolution, catalog ID filters, broad remote traversal, and threaded catalog crawling are deferred. |
...
| category | LOC | files |
|---|---:|---:|
| port-candidate | 13,793 | 143 |
| c-abi-backed | 41,804 | 368 |
| native-adapter | 2,921 | 23 |
| holdout | 1,279 | 5 |
| total | 59,797 | 539 |

Port-candidate backlog by area: `io` 7,461 · `pdal` 5,179 · `filters` 1,000 ·
`kernels` 153. `apps` and `tools` are now at 0 (apps is a thin entry-point
