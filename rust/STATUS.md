...
- EPT/COPC/STAC/remote breadth: transformed EPT preview point counts,
  semantically same-SRS transformed preview bounds clipping, and local
...
- `pdal_io_ept_reader_test`: `inspect`, `inspectBounds`, `fullReadLaszip`,
  ...
  `writers.ept_addon` — keep working with the Rust read path. True cross-SRS
  transformed preview bounds clipping remains open.
