...
use pdal_io::optech::OptechReader;
...
fn installed_pdal_matches_rust_optech_pipeline() {
    let repo = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input = repo.join("test/data/optech/sample.csd");
    let temp = make_temp_dir("optech-regression");
...
fn run_rust_pipeline(input: &Path, output: &Path) {
    ...
    let reader = pipeline.add_reader(
        "readers.optech",
        Box::new(OptechReader::new(&reader_options)),
        reader_options,
    );
...
#[test]
fn test_rust_optech_pipeline_standalone() {
    let repo = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input = repo.join("test/data/optech/sample.csd");
    let temp = make_temp_dir("optech-standalone");
    let rust_output = temp.join("rust.pcd");
    run_rust_pipeline(&input, &rust_output);
