...
| Raster writer | in progress | Raster attachments on Rust point views can write through the Rust C ABI, and `pdal_filters_faceraster_test` now exercises the Rust-backed `writers.raster` wrapper. Named/multi-raster output and the C++ `data_type` option route through Rust for the covered GDAL raster attachment paths. Broader GDAL raster option/type parity remains open. |
