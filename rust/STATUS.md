...
| ILVIS2 reader | in progress | Existing C++ reader and metadata-sidecar unit-test shapes pass through the Rust-backed path for deterministic ASCII point and fixture-shaped XML metadata behavior. Point input can open local files and GDAL VSI/HTTP(S) text sources; metadata sidecar remains local-path based. |
