...
| category | LOC | files |
|---|---:|---:|
| port-candidate | 22,830 | 208 |
| c-abi-backed | 38,242 | 339 |
| native-adapter | 1,428 | 10 |
| holdout | 339 | 4 |
| total | 62,839 | 561 |

Port-candidate backlog by area: `pdal` 9,332 · `io` 8,010 · `filters` 3,436 ·
`kernels` 2,052. `apps` and `tools` are now at 0 (apps is a thin entry-point
...
`GroundKernel`/`MergeKernel`/`RandomKernel`/`SortKernel`/`SplitKernel`/
`TileKernel` execution through `pdal_rust_kernel_run`, reducing kernel
port-candidate backlog while retaining the exported classes as compatibility
shells.)
