...
fn installed_pdal_matches_rust_qfit_decimation_pipeline() {
    let repo = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input = repo.join("test/data/qfit/10-word.qi");
    let temp = make_temp_dir("qfit-decimation-regression");
    ...
    fs::write(
        ...
        format!(
            r#"[
  {{"type":"readers.qfit","filename":"{}"}},
  {{"type":"filters.decimation","step":100}},
  {{"type":"writers.text","filename":"{}","order":"X,Y,Z,OffsetTime,StartPulse,ReflectedPulse,Azimuth,Pitch,Roll","quote_header":false,"precision":3}}
]
"#,
...
fn run_rust_pipeline(input: &Path, output: &Path) {
    ...
    filter_options.add("step", 100);
    let mut writer_options = Options::new();
    writer_options
        .add("filename", output.display())
        .add(
            "order",
            "X,Y,Z,OffsetTime,StartPulse,ReflectedPulse,Azimuth,Pitch,Roll",
        )
    ...
    let reader = pipeline.add_reader(
        "readers.qfit",
        Box::new(QfitReader::new(&reader_options)),
        reader_options,
    );
...
#[test]
fn test_rust_qfit_pipeline_standalone() {
    let repo = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input = repo.join("test/data/qfit/10-word.qi");
    let temp = make_temp_dir("qfit-standalone");
    let rust_output = temp.join("rust.txt");
    run_rust_pipeline(&input, &rust_output);
