...
| Unsafe Rust footprint | in progress | Current first-party Rust count, excluding `rust/target`, is 358 `unsafe { ... }` blocks, 568 `unsafe extern "C" fn` exports, 88 total `unsafe fn` helpers, three unsafe extern callback type aliases, one unsafe extern block, and two `unsafe impl`s. Unsafe remains concentrated in `pdal-capi`, `pdal-native`, and Rust callers of the C ABI; keep new unsafe at C/native boundaries or tests that exercise those boundaries. |
...
- `pdal_spatial_reference_test`: `test_ctor`, `calcZone`, `wgs84FromZone`,
  ...
  like `OGRGeometry::transform`; GeoTIFF VLR decoding routes through
  `pdal-native` libgeotiff, while GeoTIFF VLR encoding remains C++
  libgeotiff-backed.
