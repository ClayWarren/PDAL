...
| PLY I/O | in progress | C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII/binary reads, ASCII/binary writes, mesh faces, precision/dim typing, and `#` flex filenames. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Broader installed-PDAL and uncommon PLY fixture coverage can still grow. |
