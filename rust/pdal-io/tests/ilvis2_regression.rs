...
use pdal_io::ilvis2::Ilvis2Reader;
...
fn installed_pdal_matches_rust_ilvis2_decimation_pipeline() {
    let repo = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let input = repo.join("test/data/ilvis2/ILVIS2_GL2009_0414_R1401_042504.TXT");
    let temp = make_temp_dir("ilvis2-decimation-regression");
...
fn run_rust_pipeline(input: &Path, output: &Path) {
    ...
    let reader = pipeline.add_reader(
        "readers.ilvis2",
        Box::new(Ilvis2Reader::new(&reader_options)),
        reader_options,
    );
