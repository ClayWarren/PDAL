...
impl ColorinterpFilter {
    ...
    fn ensure_bands(&mut self) -> Result<(), StageError> {
        if self.red_band.is_empty() {
            ...
            // In PDAL, this opens a /vsimem file or a real file.
            // For the spike, we'll assume the ramp is a file path for now.
            // (The C++ side handles the memory mapping of built-in ramps).
            let raster = Raster::open(&self.ramp).map_err(StageError)?;

            // Assume fixed width for simplicity or query it?
            // ColorInterpRamps are usually 256x1 or similar.
            let width = 256;
            let height = 1;
