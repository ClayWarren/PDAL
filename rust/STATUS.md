...
| PLY I/O | in progress | C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII/binary reads, ASCII/binary writes, mesh faces, precision/dim typing, and `#` flex filenames. Binary vertex-only reader input plus ASCII/binary single-file vertex writer output are streamable in the Rust pipeline executor, with the writer using a temporary row stream so the final vertex count remains correct; ASCII reader input, mesh/list reads, mesh-face writes, and `#` fan-out writes remain materialized. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Broader installed-PDAL and uncommon PLY fixture coverage can still grow. |
