...
| GitHub workflows | in progress | The Pixi workflow runs the Rust workspace guard, the C++ implementation-replacement audit, the C++ build, an installed `PDAL::CAPI` consumer smoke, the C++ test-parity audit, and the C++ tests. Shared CI conda environments and Docker/Alpine build images install Rust/Cargo explicitly for CMake builds that require the Rust C ABI. Linux, macOS, Windows, conda, and release workflows still need equivalent Rust/C++ parity gates before the port is upstreamable. |
