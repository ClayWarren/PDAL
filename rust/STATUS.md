...
- EPT/COPC/STAC/remote breadth: transformed EPT preview point counts and local
  polygon/OGR preview point counts are Rust-pruned, but transformed preview
  bounds clipping, origin preview fallback, broad remote traversal, schema
  validation, object-store option parity, and accepted addon limitations
  remain.
...
- `pdal_io_ept_reader_test`: `inspect`, `inspectBounds`, `fullReadLaszip`,
  ...
  preview bounds clipping and origin preview remain C++ fallback.
