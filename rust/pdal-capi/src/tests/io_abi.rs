...
fn test_io_abi_nulls_and_errors() {
    unsafe {
        ...
        let err_shape = pdal_memoryview_shape_parse(std::ptr::null(), std::ptr::null_mut(), std::ptr::null_mut(), std::ptr::null_mut());
        assert!(!err_shape.is_null());
        take_string(err_shape);

        // --- 3. MemoryView read nulls & errors ---
        assert!(pdal_memoryview_read(std::ptr::null(), 5, None, std::ptr::null_mut(), 0, 0, 0, false).is_null());
        let fields = [pdal_memoryview_field_t { name: std::ptr::null(), type_id: 0x408, offset: 0 }];
        assert!(pdal_memoryview_read(fields.as_ptr(), 1, Some(memory_incrementer), std::ptr::null_mut(), 0, 0, 0, false).is_null());

        let name = CString::new("X").unwrap();
        let fields_bad_type = [pdal_memoryview_field_t { name: name.as_ptr(), type_id: 0x999, offset: 0 }];
        assert!(pdal_memoryview_read(fields_bad_type.as_ptr(), 1, Some(memory_incrementer), std::ptr::null_mut(), 0, 0, 0, false).is_null());
        ...
        assert!(!pdal_writer_write_view(std::ptr::null_mut(), std::ptr::null()));
        assert!(!pdal_writer_write_views(std::ptr::null_mut(), std::ptr::null(), 0));
        ...
        assert!(pdal_ept_reader_preview_create(invalid_filename.as_ptr() as *const c_char).is_null());
        ...
        let mut minx = 0.0; let mut miny = 0.0; let mut minz = 0.0;
        let mut maxx = 0.0; let mut maxy = 0.0; let mut maxz = 0.0;
        assert!(!pdal_ept_reader_preview_bounds(std::ptr::null(), &mut minx, &mut miny, &mut minz, &mut maxx, &mut maxy, &mut maxz));
