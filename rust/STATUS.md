...
| Metadata | in progress | Typed scalar metadata trees, descriptions, instance/array node kind preservation, scalar JSON helpers, and JSON serialization exist. JSON/base64 typed nodes and full pipeline serialization remain incomplete. |
...
| TIndex reader | in progress | GeoJSON tile-index reads can route through the Rust C ABI from local files and GDAL VSI/HTTP(S) text sources, and the C++ wrapper uses that path for simple indexes with optional bounding-box filtering when `t_srs` is explicitly empty. Basic local OGR datasource reads now use the Rust GDAL adapter for layer-0 or named-layer string location fields, OGR `where` attribute filters, OGR SQL result layers, geometry-envelope bounds filtering, and same-SRS WKT polygon filters. Reprojection, SRS-column handling, and reader-args remain on the C++ path. |
