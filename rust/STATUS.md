...
Current pre-port checkpoint: `745 / 891` baseline C++ GoogleTest cases, or
`83.61%`, are confirmed Rust C ABI-backed by
...
added during the port, is `779 / 932` currently built C++ GoogleTest cases, or
`83.58%`; compute that with `--include-added-tests`.
...
installed-app `sort`/`merge`, and `Utils::run_shell_command()` through the
Rust C ABI. This remains a conservative lower bound, not a final
...
current build. Of these:
   ...
   - 6 are command/infrastructure/utility/tooling tests, not pipeline stages
     (app, artifact, info cmd, program_arg, tile cmd, tindex cmd)
