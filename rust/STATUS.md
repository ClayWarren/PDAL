...
| `package.sh` and release packaging | in progress | Source packaging keeps Rust sources while excluding `rust/target/`, and `package.sh` installs Rust/Cargo alongside the C++ build tools. Release packaging still needs license/vendor accounting and generated-artifact review before upstream release use. |
