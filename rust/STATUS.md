...
| PCD I/O | in progress | Existing C++ reader/writer unit-test shapes pass through the Rust-backed path, including ASCII, binary, binary-compressed, precision, streaming, and double-field coverage. ASCII reader input and writer output are streamable in the Rust pipeline executor; the writer uses a temporary row stream so the final `POINTS` header remains correct. Binary and binary-compressed reads/writes remain materialized. Reader input can open local files and GDAL VSI/HTTP(S) byte sources. Broader installed-PDAL parity coverage can still grow. |
